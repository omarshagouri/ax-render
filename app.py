...
CLIPS_FOLDER_ID = os.environ.get("CLIPS_FOLDER_ID", "1ygVMe7TpyR8zFugivTvY6Lrkthah3owo")
...
# ---- clips (stock B-roll) -------------------------------------------------
# A VV-SF-### beat is stock footage, not a card. We turn it into a beat MP4 that
# is byte-format-identical to a rendered card beat (1080x1920, yuv420p, 30fps,
# h264, NO audio) so Agent 5's save-to-folder and the assembly stage need no
# change. The source file is resolved from the Drive "clips" folder by name.
_clip_fid_cache = {}      # clip_id -> Drive file id
_clips_index = None       # {filename: file_id} for the clips folder, built once

def _build_clips_index():
    """List every .mp4 in the clips folder once (cached per warm instance)."""
    drive = _drive_client()
    index, page_token = {}, None
    while True:
        res = drive.files().list(
            q=f"'{CLIPS_FOLDER_ID}' in parents and trashed = false",
...
def resolve_clip_file_id(clip_id):
    """Map 'VV-SF-001' to a Drive file id. Prefer an exact 'VV-SF-001.mp4';
    otherwise any .mp4 whose name contains the id (e.g. 'VV-SF-001_charging.mp4').
    Rebuilds the folder index once on a miss so newly added clips are picked up."""
    global _clips_index
    if clip_id in _clip_fid_cache:
        return _clip_fid_cache[clip_id]
    cid = clip_id.upper()
    for attempt in range(2):
        if _clips_index is None or attempt == 1:
            _clips_index = _build_clips_index()
        match = None
        for name, fid in _clips_index.items():
            if name.upper() == cid + ".MP4":
                match = fid
                break
        if match is None:
            for name, fid in _clips_index.items():
                if cid in name.upper():
                    match = fid
                    break
        if match:
            _clip_fid_cache[clip_id] = match
            return match
    raise HTTPException(404, f"clip {clip_id}: no matching .mp4 in clips folder {CLIPS_FOLDER_ID}")

async def render_clip(clip_id, duration, out_path, clip_file_id=None, fps=30):
    """Download the source clip, trim to `duration`, scale-crop to fill 9:16, and
    re-encode to the exact card-beat spec (h264 / yuv420p / 30fps / no audio)."""
    file_id = clip_file_id or resolve_clip_file_id(clip_id)
    fd, src = tempfile.mkstemp(suffix=".src.mp4")
    os.close(fd)
    try:
        download_drive_file_to_path(file_id, src)
        vf = ("scale=1080:1920:force_original_aspect_ratio=increase,"
              "crop=1080:1920,fps=30,format=yuv420p")
        subprocess.run(
            f'ffmpeg -y -i "{src}" -t {duration} -vf "{vf}" '
            f'-an -c:v libx264 -pix_fmt yuv420p -r {fps} '
...
async def render_beat(req: Request):
    ...
    # ---- B-roll clip beats (VV-SF-###): produce a beat MP4 identical in format
...
async def thumbnail(req: Request):
    ...
    if bg_b64:
        bg_uri = "data:image/png;base64," + bg_b64.strip()
    elif bg_file_id:
        try:
    ...
    if logo_b64:
        logo_uri = "data:image/png;base64," + logo_b64.strip()
    elif logo_file_id:
        try:
