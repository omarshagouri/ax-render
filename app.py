...
async def startup():
    ...
    global _pw, _browser, BG_URI, _drive
    ...
    print("startup complete: browser warm, background loaded, drive ready")
...
def upload_to_drive(local_path, filename, folder_id):
    ...
    return _drive.files().create(body=meta, media_body=media, fields="id").execute()["id"]
...
async def render_beat(req: Request):
    ...
    out_folder_id = body["out_folder_id"]
    ...
    file_id = upload_to_drive(out_path, filename, out_folder_id)
    os.remove(out_path)

    return {"status": "ok", "file_id": file_id, "filename": filename, "duration": duration}
