...
Contract (Make -> service), same x-api-key header as /render-beat:
  ...
  {
    "video_id": "AX-020-SF",
    "out_folder_id": "<Rendered col F>",
    "intro_photo_file_id": "<Rendered col I>",   # optional: 1s silent still at the start
    "outro_video_file_id": "<Rendered col H>",   # optional: end clip, keeps its own audio
...
def _list_clips(svc, folder_id):
    ...
    while True:
        resp = svc.files().list(
            q=f"'{folder_id}' in parents and trashed=false and name contains '.mp4'",
...
def _build(video_id, folder_id, beats_in, chapters_in,
           intro_photo_file_id=None, outro_video_file_id=None):
    ...
    try:
        ...
        # +++ optional intro still + outro clip (Drive reads via the SA) +++
        intro_id = (intro_photo_file_id or "").strip()
        outro_id = (outro_video_file_id or "").strip()
        intro_path = _download(svc, intro_id, os.path.join(work, f"{video_id}_intro")) if intro_id else None
        outro_path = _download(svc, outro_id, os.path.join(work, f"{video_id}_outro.mp4")) if outro_id else None
        # +++ end +++

        out_path = os.path.join(work, f"{video_id}_FINAL.mp4")
        info = assemble({"beats": m_beats, "chapters": m_chaps,
                         "intro_photo": intro_path, "outro_video": outro_path}, work, out_path)
...
async def assemble_video(req: Request):
    ...
    filename, b64, info = await loop.run_in_executor(
        None, _build,
        body["video_id"], body["out_folder_id"], body["beats"], body["chapters"],
        body.get("intro_photo_file_id"), body.get("outro_video_file_id"))
