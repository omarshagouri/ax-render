...
FONT_SIZE       = int(os.environ.get("CAPTION_FONT_SIZE", "150"))       # at 1080x1920
...
OUTLINE_PX      = os.environ.get("CAPTION_OUTLINE", "8")
SHADOW_PX       = os.environ.get("CAPTION_SHADOW", "4")
POS_X           = int(os.environ.get("CAPTION_POS_X", "540"))           # 1080/2
POS_Y           = int(os.environ.get("CAPTION_POS_Y", "1200"))          # a touch below middle (matches ref)
MAX_WORDS       = int(os.environ.get("CAPTION_MAX_WORDS", "3"))         # words visible per group
MAX_CHARS       = int(os.environ.get("CAPTION_MAX_CHARS", "18"))        # per group line
...
def _group_words(words):
    ...
    for w in words:
        disp = w[0].upper() if UPPERCASE else w[0]
        ...
        if _END_RE.search(w[0].strip()):     # flush on sentence enders for clean breaks
            groups.append(cur); cur, cur_chars = [], 0
    if cur:
        groups.append(cur)
    return groups
...
Style: Kar,{FONT_NAME},{FONT_SIZE},{white},{white},&H00101010,&H80000000,-1,0,0,0,100,100,0,0,1,{OUTLINE_PX},{SHADOW_PX},5,60,60,0,1
...
"""
    ...
    for g in groups:
        # one Dialogue per word: that word teal, the rest white. The line stays put -> "pop" highlight.
        for i in range(len(g)):
            ...
            text = " ".join(parts)
            lines.append(f"Dialogue: 0,{_ts_ass(start)},{_ts_ass(end)},Kar,,0,0,0,,{{{pos}}}{text}")
            event_count += 1
    return header + "\n".join(lines) + "\n", event_count
